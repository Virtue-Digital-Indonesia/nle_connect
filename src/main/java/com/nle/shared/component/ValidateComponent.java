...
public class ValidateComponent {
    ...
    public DepoOwnerAccount ValidatePhoneNumberToken (Optional<String> username, Long request_depo_id){
        if (username.isEmpty())
    ...
    public DepoOwnerAccount ValidateDepoToken (Optional<String> username){
        if (username.isEmpty())
            throw new BadRequestException("Invalid Token!");
