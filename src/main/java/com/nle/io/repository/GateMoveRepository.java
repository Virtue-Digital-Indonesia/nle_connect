...
public interface GateMoveRepository extends JpaRepository<GateMove, Long> {
    ...
    Page<GateMove> findAllByDepoOwnerAccount_CompanyEmailAndGateMoveType(String depoOwnerAccount, String gateMoveType,
            Pageable pageable);
    ...
    @Query("select new com.nle.io.repository.dto.ShippingLineStatistic(gm.fleet_manager, count (gm.fleet_manager)) " +
            "from GateMove gm " +
            "where tx_date between CURDATE() - INTERVAL :from DAY AND CURDATE() - INTERVAL :to DAY " +
            "group by gm.fleet_manager " +
