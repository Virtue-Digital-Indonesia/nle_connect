...
public class ImpersonateController {
    ...
    public ResponseEntity<JWTToken> forcedIn(HttpServletResponse response) {
        String jwtToken = response.getHeader("token");
        return ResponseEntity.ok(new JWTToken(jwtToken));
    }

    @Operation(description = "check token", operationId = "findByCode", summary = "check token meaning")
    @SecurityRequirement(name = "nleapi")
